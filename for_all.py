...
# here we also give x_test as an arguement to the function for last part of the assignment we can remove it since other parts does not need this 
    ...
    np.savetxt(Dataset_dir + index + "_y_predict_test.txt", y_predict_test) # only for part d)
...
plt.yticks(list(np.linspace(0,max(val_mse_array),5)) + val_mse_array)
plt.legend()
...
plt.savefig(Dataset_dir + index +'_Train_and_validation_MSE_for_n=1_to_10.png')
plt.show()
